...
export const CardGridLoader: React.FC = React.memo(() => {
  const [items, setItems] = React.useState<ElderCard[]>(mockCards);
  ...
  const isItemLoaded = React.useCallback(index => !!items[index], [items]);
    ...
    <Autosizer>
      {({ height, width }) => (
        ...
        >
          {({ onItemsRendered, ref }) => (
            <CardGrid
              {...computeAvailableSpace({ height, width, itemCount })}
              innerRef={ref}
              items={items}
              itemCount={itemCount}
